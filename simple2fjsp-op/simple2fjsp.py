...
class JobEnv:
    def __init__(self, case_name, path, only_PDR=False, use_make_span=False):
    ...
    def get_selection(self, action):
        ...
        job_PDR = int(action / self.num_of_machine_pdr)
        machine_PDR_index = action % self.num_of_machine_pdr
        machine_selection = self.machine_PDR[machine_PDR_index]

        return job_PDR, machine_selection
    ...
    def step(self, action):
        ...
        if self.stop():
            if self.use_make_span:
                self.reward = -self.current_time
            self.done = True
    ...
    def allocate_job(self, job_id, sign):
        ...
        for t in range(self.num_of_machine_pdr):
            self.mask[job_id * self.num_of_machine_pdr+t] = -1e8
        ...
        for x in range(self.job_num):
            if not self.busy_job[x]:
                ...
                if not self.assignable(x, self.current_op_of_job[x]):
                    ...
                    for t in range(self.num_of_machine_pdr):
                        self.mask[x * self.num_of_machine_pdr + t] = -1e8
        ...
        for k in range(self.machine_num):
            ...
            if cur_job_id >= 0 and self.current_time >= self.next_time_on_machine[k]:
                ...
                for x in range(self.job_num):  # release jobs on this machine
                    if not self.busy_job[x] and not self.finished_jobs[x]:
                        ...
                        if self.assignable(x, self.current_op_of_job[x]):
                            ...
                            for t in range(self.num_of_machine_pdr):
                                self.mask[x * self.num_of_machine_pdr + t] = 0
                ...
                if self.current_op_of_job[cur_job_id] >= self.orders_of_job[cur_job_id]:
                    ...
                    for t in range(self.num_of_machine_pdr):
                        self.mask[cur_job_id * self.num_of_machine_pdr + t] = -1e8
                else:
                    ...
                    if not self.assignable(cur_job_id, self.current_op_of_job[cur_job_id]):
                        ...
                        for t in range(self.num_of_machine_pdr):
                            self.mask[cur_job_id * self.num_of_machine_pdr + t] = 0
